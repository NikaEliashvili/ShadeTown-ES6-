...
function render(sunglassesNew) {
  ...
  productDetailsEl.innerHTML = `<h1>${sunglassesNew.model.name} </h1><p>Custom: ${sunglassesNew.lenses.color} lenses, ${sunglassesNew.frame.color} frames</p>
  <p>${price}</p>`;
...
document.body.addEventListener("click", function (event) {
  ...
  if (clickedItem.classList.contains("product-color-swatch")) {
    ...
    if (clickedItem.closest("div").classList[0] === "product-lenses") {
      let colorOptions = sunglassesOptions.lenses.filter(function (item) {
    ...
    //frames
      ...
      let colorOptions = sunglassesOptions.frames.filter(function (item) {
        return item.color === currColor;
      })[0];
